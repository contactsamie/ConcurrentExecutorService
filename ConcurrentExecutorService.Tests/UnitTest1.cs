...
namespace ConcurrentExecutorService.Tests
{
    public class UnitTest1
    {
        ...
        public void it_should_run_many_operations_fast()
        {
            ...
            const int maxTotalExecutionTimeMs = 5000;
            var service = new ConcurrentExecutorServiceLib.ConcurrentExecutorService(maxExecutionTimePerAskCall);
            var watch = System.Diagnostics.Stopwatch.StartNew();
            Parallel.ForEach(Enumerable.Range(0, numberOfRequests), (basket) =>
            {
                var result = service.GoAsync( () => Task.FromResult(new object()), basket.ToString()).Result;
            });
            watch.Stop();
            var elapsedMs = watch.ElapsedMilliseconds;

           Assert.True(elapsedMs< maxTotalExecutionTimeMs, $"Test took {elapsedMs} ms which is more than {maxTotalExecutionTimeMs}");
        }
        [Fact]
        public void it_should_run_one_operations_sequentially()
        {
            ...
            var durationMs = TestHelper.TestOperationExecution(   numberOfBaskets, numberOfPurchaseFromOneBasketCount, maxExecutionTimePerAskCall);
